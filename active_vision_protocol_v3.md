...
The smoke uses 8 new train-only layout groups, four hidden states per group, and both route preferences. Since eight groups cannot be evenly allocated across all six registered Phase 5B families, the smoke allocation is fixed in advance as two groups each from `F0`, `F1`, `F2`, and `F3`; `F4` and `F5` are excluded from this smoke. The smoke manifest seed is `54040` and its scene dynamics seed is `20260920`. It does not train a selector, tune a view policy, or open validation/test. With one proposer inference per trial and at most three fixed-candidate route rollouts, the expected budget is 64–192 proposer/route units plus exact replay audits; actual counts are logged rather than inferred.
