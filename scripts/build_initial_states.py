...
def state_hash(state):
    blob = json.dumps(state, sort_keys=True, default=lambda value: value.tolist() if hasattr(value, "tolist") else str(value))
...
for task_id in range(suite.n_tasks):
    ...
    for name, count in ALLOC.items():
        ...
        start += count
        entry[name] = [{"init_index": index, "state_hash": state_hash(states[index])} for index in selected]
